...
M.form_filemanager.init = function(Y, options) {
    ...
    Y.extend(FileManagerHelper, Y.Base, {
	    ...
	    // Custom code start
            var formElement = Y.one('.mform');
            if (formElement.hasClass('custom-form-class')) {
                this.selectnode = Y.Node.create(M.form_filemanager.templates.fileselectlayoutcustom);
            } else {
                this.selectnode = Y.Node.create(M.form_filemanager.templates.fileselectlayout);
            }
            // Custom code end
	    ...
	    // Custom code start
            var selectrolenode = Y.one('.fp-accessrole');

            // Check if the selectrolenode exists
            if (selectrolenode) {
                // Get the select element
                var selectElement = selectrolenode.one('select');

                if (selectElement) {
                    // Get the <select> DOM node directly
                    var selectNode = selectElement._node;  // This gets the raw DOM node from YUI Node

                    // Now get the selected options using JavaScript's DOM method
                    var selectedValues = [];
                    ...
                    // Now selectedValues contains an array of all selected values
                    console.log(selectedValues);

                    // Compare if the role has changed (just like your existing logic)
                    var newrole = selectedValues.join(', '); // Join values as a comma-separated string
                    var rolechanged = (newrole !== fileinfo.accessrole);
                }
            } else {
                var newrole = '';
            ...
            if (fileinfo.type == 'folder') {
                ...
                if (filenamechanged || filepathchanged || licensechanged || authorchanged || rolechanged) {
                    params = {filepath:fileinfo.filepath, filename:fileinfo.fullname,
                        newfilename:newfilename, newfilepath:targetpath,
                        newlicense:newlicense, newauthor:newauthor, newrole: newrole};
                    action = 'updatefile';
                }
            }
            if (!action) {
        ...
        setup_select_file: function() {
            ...
            selectnode.all('.fp-saveas,.fp-path,.fp-author,.fp-license,.fp-role').each(function (node) {
	    ...
	    // Custom code start
            var selectrolenode = Y.one('.fp-accessrole');
            if (selectrolenode) {
                var selectElement = selectrolenode.one('#accessrole'); // Get the select element

                // Assuming node.role is a comma-separated string of values
                var selectedValues = node.accessrole.split(', '); // Split into array of selected roles

                // Loop through the YUI NodeList of options
                selectElement.get('options').each(function(option) {
                    // Check if the option's value is in the selectedValues array
                    if (selectedValues.includes(option.get('value'))) {
                        option.set('selected', true); // Mark this option as selected
                    } else {
                        option.set('selected', false); // Deselect if it's not part of selected values
                    }
                });
            }
            // Custom code end
            ...
            selectnode.all('.fp-author input, .fp-license select, .fp-accessrole select').set('disabled',(node.type == 'folder')?'disabled':'');
