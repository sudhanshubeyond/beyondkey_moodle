...
class block_edvik_blog_area extends block_base {
    public function init() {
        $this->title = get_string('pluginname', 'block_edvik_blog_area');
    }
            ...
            <div class="row align-items-center mb-30">
                <div class="col-lg-8">
                    <div class="section-title style-four">
                        ...
                        <h2 class="d-inline-block font-semibold position-relative mb-0">'.format_text($this->content->title, FORMAT_HTML, array('filter' => true)).' ';
                            if($this->config->title_shape):
                                $text .= '<img class="position-absolute bottom-0 end-0" src="'.edvik_block_image_process($this->config->title_shape).'" alt="'.strip_tags($this->content->title).'">';
                            endif; 
                            $text .= '
                        </h2>
                    </div>
                ...
                <div class="col-lg-4 text-md-end mt-sm-20">';
                    ...
                    if(!empty($this->content->button_text) && !empty($this->content->button_link)){
                        $text .= '
                            <a href="'.$this->content->button_link.'" class="link style-one">'.format_text($this->content->button_text, FORMAT_HTML, array('filter' => true)).' <i class="ri-arrow-right-line"></i></a>
                        ';
                    }
                    $text .= '
                </div>
                ...
                <div class="swiper-wrapper">';
                    if($this->content->posts):
                        foreach ($entries as $entryid => $entry) {
                            ...
                            $wordCount = str_word_count($short_summary);
                            ...
                            if(in_array($entry->id, $this->content->posts)):
                                ...
                                <div class="swiper-slide">
                                    <div class="blog-card bg-mystic style-one d-flex flex-wrap align-items-center round-10 mb-40">
                                        <div class="blog-img round-10 overflow-hidden">
                                            <img src="https://edvik-moodle.hibootstrap.com/theme/edvik/pix/instructor/instructor-thumb-8.webp" alt="'.strip_tags($entry->subject).'" class="round-10 transition">
                                        </div>
                                        <div class="blog-info">
                                            ...
                                            <div class="blog-author d-flex align-items-center">
                                                <div class="author-info fs-14 ls-1">
                                                    '.$this->content->by_title.' <a href="'.$viewblogurl.'" class="ls-1">'.$entry->firstname.' '.$entry->lastname.'</a>
                                                </div>
                                            </div>
                                            <a href="'.$viewblogurl.'" class="link style-two fs-15">Read More<i class="ri-arrow-right-line"></i></a>
                                        </div>
                                    </div>
                                </div>';
                            endif;
