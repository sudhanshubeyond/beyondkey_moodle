    ...
    function($, Ajax, Notification, Str) {
        ...
        return {
            async setup(props, modelurl) {
                ...
                // Robust attempt id resolution: prefer props.attemptid, then props.attempt,
                // then fallback to URL query param "attempt".
                const attemptid = props.attemptid ?? props.attempt ?? (function() {
                    try {
                        const sp = new URLSearchParams(window.location.search);
                        const a = sp.get('attempt');
            ...
            async init(props) {
                // Resolve attempt id here as well for init() path.
                const attemptid = props.attemptid ?? props.attempt ?? (function() {
                    try {
                        const sp = new URLSearchParams(window.location.search);
                        const a = sp.get('attempt');
