...
class quizaccess_proctoring_external extends external_api {
    ...
    public static function send_camshot_parameters() {
        return new external_function_parameters(
            [
                ...
                'attempt' => new external_value(PARAM_INT, 'Attempt', VALUE_DEFAULT, 4)
            ]
        );
    }
    ...
    public static function send_camshot
        ($courseid, $screenshotid, $quizid, $webcampicture, $imagetype, $parenttype, $faceimage, $facefound, $attempt) {
        global $DB, $USER;
        ...
        if ($imagetype == 1) {
            ...
            if ($faceimage) {
                ...
                // Upload face image to Azure Blob Storage (if configured).
                $faceraw = base64_decode(explode(',', explode(';', $faceimage)[1])[1]);
                $faceazurepath = "faces/{$USER->id}/face_{$USER->id}_{$screenshotid}_" . time() . ".png";
                quizaccess_proctoring_upload_to_azure($faceraw, $faceazurepath);
    ...
    public static function validate_face_parameters() {
        return new external_function_parameters(
            [
                ...
                'quizid' => new external_value(PARAM_RAW, 'quiz id'),
     ...
     */
    public static function validate_face($courseid, $cmid, $profileimage, $quizid, $attemptid, $webcampicture, $parenttype, $faceimage, $facefound) {
        global $DB, $USER, $CFG;
        ...
        $url = self::geturl($data, $screenshotid, $USER, $courseid, $record, $context, $fs);
            ...
            // Upload webcam image to Azure Blob Storage (if configured).
            $webcamraw2 = base64_decode(explode(',', $webcampicture)[1] ?? '');
            if ($webcamraw2) {
                $azureblobpath2 = "quizzes/$quizid/{$USER->id}/$attemptid/New/webcam_{$USER->id}_{$screenshotid}_" . time() . ".png";
                quizaccess_proctoring_upload_to_azure($webcamraw2, $azureblobpath2);
            }
        ...
        if ($faceimage) {
            ...
            $url = self::quizaccess_proctoring_geturl_without_timecode(
                ...
                // Upload face image to Azure Blob Storage (if configured).
                $faceraw2 = base64_decode(explode(',', explode(';', $faceimage)[1])[1] ?? '');
                if ($faceraw2) {
                    $faceazurepath2 = "faces/{$USER->id}/face_{$USER->id}_{$screenshotid}_" . time() . ".png";
                    quizaccess_proctoring_upload_to_azure($faceraw2, $faceazurepath2);
                }
