...
/**
 * Upload a binary image to Azure Blob Storage using Shared Key authentication.
 *
 * Credentials are read from Moodle plugin config (never hardcoded).
 * Configure via: Admin > Plugins > Quiz access rules > Proctoring > Azure settings.
 *
 * @param string $imagedata  Raw binary image data (already base64-decoded).
 * @param string $blobpath   Full blob path inside the container, e.g. "Students/42/face_42_1234567890.png".
 * @return string|false      The public blob URL on success, false on failure.
 */
function quizaccess_proctoring_upload_to_azure(string $imagedata, string $blobpath) {
    $accountname   = get_config('quizaccess_proctoring', 'azure_account_name');
    $accountkey    = get_config('quizaccess_proctoring', 'azure_account_key');
    $containername = get_config('quizaccess_proctoring', 'azure_container_name');

    // Silently skip if Azure is not configured — plugin still works without it.
    if (empty($accountname) || empty($accountkey) || empty($containername)) {
        return false;
    }

    // Write image data to a temp file for streaming upload.
    $tempfile = tempnam(sys_get_temp_dir(), 'proctoring_azure_');
    if ($tempfile === false || file_put_contents($tempfile, $imagedata) === false) {
        debugging('quizaccess_proctoring: Failed to write temp file for Azure upload.', DEBUG_DEVELOPER);
        return false;
    }

    $filesize = filesize($tempfile);
    $endpoint = "https://{$accountname}.blob.core.windows.net/{$containername}/{$blobpath}";
    $date     = gmdate('D, d M Y H:i:s T');

    // Build Azure Shared Key signature string exactly as documented by Microsoft.
    $stringtosign =
        "PUT\n\n\n{$filesize}\n\nimage/png\n\n\n\n\n\n\n" .
    ...
    if ($fh === false) {
        debugging('quizaccess_proctoring: Failed to open temp file for Azure upload.', DEBUG_DEVELOPER);
        @unlink($tempfile);
        return false;
    }
    ...
    if ($status === 201) {
        debugging("quizaccess_proctoring: Azure upload OK → {$endpoint}", DEBUG_DEVELOPER);
        return $endpoint;
    }

    debugging("quizaccess_proctoring: Azure upload FAILED (HTTP {$status}): {$response}", DEBUG_DEVELOPER);
    return false;
}
